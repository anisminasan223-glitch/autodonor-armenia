...
// The site is intentionally served from the repository root so index.html
// works with the simple GitHub repository layout used by this project.
app.use(express.static(__dirname, { extensions: ['html'] }));

app.post('/api/lead', upload.array('media', MAX_FILES), async (req, res) => {
  const files = req.files || [];
  try {
    if (!mailer) throw new Error('Почта не настроена. Заполните MAIL_USER и MAIL_PASS в .env.');

    const { name = '', phone = '', brand = '', model = '', details = '' } = req.body;
    const subject = `Новая оценка автомобиля — ${String(brand).slice(0,100) || 'автомобиль'} ${String(model).slice(0,100)}`.trim();
    const text = [
      'НОВАЯ ЗАЯВКА — AutoDonor Armenia',
      '',
      `Имя: ${String(name).slice(0, 200)}`,
      `Телефон: ${String(phone).slice(0, 100)}`,
      `Марка: ${String(brand).slice(0, 100)}`,
      `Модель / год: ${String(model).slice(0, 100)}`,
    ...
    res.json({ ok: true, emailSent: true, filesAttached: files.length });
  } catch (err) {
